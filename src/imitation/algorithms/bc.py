...
class BC(algo_base.DemonstrationAlgorithm):
    ...
    def set_demonstrations(self, demonstrations: algo_base.AnyTransitions) -> None:
        ...
        self.forget_set = demonstrations
    def set_demo_uni(self, uni_set: algo_base.AnyTransitions) -> None:
        self.uni_set = uni_set
    ...
    def train(
        ...
        *,
        ...
        if num_samples_so_far % self.batch_size != 0:
            ...
            batch_num += 1
            process_batch()
    def obtain_logits(self, policy: policies.ActorCriticPolicy,obs: Union[types.AnyTensor,types.DictObs,Dict[str, np.ndarray],Dict[str, th.Tensor],]):
        tensor_obs = types.map_maybe_dict(
        util.safe_to_tensor,
        types.maybe_unwrap_dictobs(obs),
        )
        return  policy.obtain_logits( tensor_obs)
    def obtain_logits_from_observation (self, obs:np.ndarray):
        return self.policy.obtain_logits_from_observation(obs)
    def assistant_train(
        self,
        *,
        n_transitions ,
        ...
        if self.uni_set is None:
            print("There is not a dataset to unlearn")
        else:
            #create dataloader and size depends on n_transitions
            assert self.forget_set is not None
            demo_forget_loader = algo_base.make_data_loader_sample(self.forget_set,self.minibatch_size, n_transitions//2)
            demo_retain_loader = algo_base.make_data_loader_sample(self.uni_set,self.minibatch_size,n_transitions//2)
            retain_batches = None
            print("There is a dataset to unlearn")
            retain_batches = BatchIteratorWithEpochEndCallback(
                demo_retain_loader,
                n_epochs,
                n_minibatches,
                _on_epoch_end,
            )
            retain_batches_stats = enumerate_batches(retain_batches)
            ...
            tqdm_progress_bar: Optional[tqdm.tqdm] = None
            if progress_bar:
                retain_batches_stats = tqdm.tqdm(
                    retain_batches_stats,
                    unit="batch",
                    total=n_minibatches,
                )
                tqdm_progress_bar = retain_batches_stats
            ...
            self.optimizer.zero_grad()
            iter_forget = iter(batches_with_stats)
            iter_retain = iter (retain_batches_stats)
            length = None
            if n_epochs == None:
